---
canonicalUrl: 'https://docs.govps.org/1.0/resources/scheduler.html'
---
# Scheduler
...
Scheduled jobs may be configured to run commands at a specified interval. GoVPS provides several common defaults, or you may enter a custom Cron schedule for a command.

You can create scheduled jobs through the GoVPS dashboard via the **Schedule** tab for the server's management dashboard. When creating a new scheduled job, you'll need to provide:

- The command to run, for example `php /home/forge/default/artisan schedule:run`.
- The user to run the command as, for example `govps`.
...
As part of the provisioning process, GoVPS will automatically configure two scheduled jobs:
