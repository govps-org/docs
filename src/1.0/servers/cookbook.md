---
canonicalUrl: 'https://docs.govps.org/1.0/servers/cookbook.html'
---
# Cookbook
...
When configuring your server, GoVPS configures FPM so that it can be restarted without using your server's "sudo" password. To do so, you should issue the following command. Of course, you should adjust the PHP version to match the version of PHP installed on your machine:

```bash
echo "" | sudo -S service php8.1-fpm reload
```

## Resetting The `govps` User Sudo Password

GoVPS does not store your server's `govps` user sudo password and is therefore unable to reset it for you. To reset the `govps` user sudo password, you'll need to contact your server provider and regain SSH access to your server as the `root` user.

Once you are connected to your server as the `root` user, you should run the `passwd govps` command to redefine the `govps` user sudo password.

#### Digital Ocean

If your servers are managed by DigitalOcean, the following steps should assist you in resetting the `govps` user's sudo password using Digital Ocean's dashboard.
...
3. Finally, execute the `passwd govps` terminal command as the `root` userto redefine the `govps` user's sudo password.

## Upgrading Composer

The latest version of Composer is installed by GoVPS when a new server is provisioned. However, as your server ages, you may wish to upgrade the installed version of Composer. You may do so using the following command:
...
The latest version of Nginx is installed by GoVPS when a new server is provisioned. However, as your server ages, you may wish to upgrade the installed version of Nginx. You may do so using the following commands:
...
The latest version of Node.js is installed by GoVPS when it is provisioning a new server. However, as your server ages, you may wish to upgrade the version of Node.js:
...
The latest version of npm is installed by GoVPS when provisioning new servers. However, you may upgrade the installed version of npm using the following commands:
...
This error is returned by [DigitalOcean](https://digitalocean.com) when you have reached a limit on how many droplets you can create. You can ask DigitalOcean to increase your droplet limit by contacting their support. Once they have increased your limit, you may create servers in GoVPS.

## AWS Provisioned Servers Are Disappearing

To ensure GoVPS works correctly with AWS, please review [these requirements](/1.0/servers/providers.html#amazon-aws-api-access).
...
- Verify that the public IP address of the server is known to GoVPS (the public IP address may change between reboots of the actual VPS).
- Verify that the GoVPS generated public key for the server is included in the `/root/.ssh/authorized_keys` and `/home/forge/.ssh/authorized_keys` files. This key is available via the "Meta" tab of your server's GoVPS management panel.
- If your server is behind a firewall, make sure you have [allowed GoVPS's IP addresses to access the server](/1.0/introduction.html#forge-ip-addresses).
- If you removed Port 22 from the server's firewall rules, you will need to contact your server provider and ask them to restore the rule. Removing this rule prevents GoVPS from accessing your server via SSH.
...
If, after trying all of the above solutions, GoVPS is still unable to connect to your server but you can still SSH to the server, please run the following command as the `root` user and share the output with GoVPS support:

```bash
grep 'sshd' /var/log/auth.log | tail -n 10
```

:::danger GoVPS Management

If GoVPS is not able to connect to your server, you will not be able to manage it through the GoVPS dashboard until connectivity is restored.
:::
