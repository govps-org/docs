---
canonicalUrl: 'https://docs.govps.org/1.0/sites/the-basics.html'
---
# The Basics
...
When creating a new site on your GoVPS server, a variety of configuration options are available to you:
...
- **Nginx Template**: If you have configured any [Nginx Templates](/1.0/servers/nginx-templates.html) for you server, you will have the option to select one as the site's Nginx configuration, otherwise GoVPS will utilise its default Nginx configuration for you new site.
...
Once the site has been created in GoVPS, you can then install an application or project. Projects contain the actual source code of your application. GoVPS can install three types of applications: an application that exists within a [Git repository](/1.0/sites/the-basics.html#git-repository), [WordPress](/1.0/sites/the-basics.html#wordpress), or [phpMyAdmin](/1.0/sites/the-basics.html#phpmyadmin). 

### Git Repository

:::tip Source Control Providers

Before you can install a Git repository, you must first ensure that you have [connected your source control provider](/1.0/accounts/source-control.html) to your GoVPS account.
:::

Once you have connected your source control provider accounts, you may choose the provider from one of the available options. If you're self-hosting your own code or using a provider that GoVPS doesn't include first-party support for, you may select the **Custom** option. When using either the Bitbucket or Custom provider option, you will need to manually add the GoVPS SSH key to the provider.
...
GoVPS can also install [WordPress](https://wordpress.org) for you, so you can get right into writing your next blog post.
...
When installing WordPress for your site, GoVPS will also install the [WordPress CLI](https://wp-cli.org/) so that you can manage your installation with the `wp` terminal command.

:::danger Securing WordPress

You should continue installing WordPress as soon as GoVPS has installed it for you, so that it's made secure with your username and password. You could also choose to create a new [Security Rule](/1.0/sites/security-rules.html) before you install WordPress so that your installation is password protected.
:::

### phpMyAdmin

GoVPS also supports installing [phpMyAdmin](https://phpmyadmin.net), so you can manage your databases from anywhere.
...
Once GoVPS has installed phpMyAdmin, you can then login to your installation using any of your database username and password combinations.
...
When you initially provision a GoVPS server, GoVPS creates a single site on the server named `default`. This site may be accessed by visiting the IP address of your server in your web browser. This is convenient because sometimes you may not have a particular domain you want to associate with a given server immediately after provisioning.
...
Below is an example of the default Nginx site configuration that is used by Laravel GoVPS. [Additional Nginx templates](/1.0/servers/nginx-templates.html) may be created in GoVPS using the "Nginx Templates" panel within your server's management dashboard.
...
If your server has [multiple versions of PHP](/1.0/servers/php.html) installed, you can switch the version used by your site at any time by using the site's **Meta** tab in the GoVPS dashboard.
...
GoVPS also provides the ability to view a site's log files from within the dashboard.

For performance reasons, GoVPS will only return the last 500 lines from a file. The log file path will be shown in both the dropdown and at the bottom of the log output.

:::tip Laravel Log Files Only

GoVPS is only able to display log files from Laravel 5+ applications. Both `daily` and `single` log formats are supported. GoVPS will automatically read the last updated file.
:::